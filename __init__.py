...
def plugin():
    with gr.Tab("推文生图工具"):
        ...
        images_number = gr.Slider(1, 999, 5, step=1, label="每段 tag 生成图片的数量")
        gr.Markdown("<hr>")
        sti_negative = gr.Textbox(
            "<negative:默认负面提示词>,", label="负面提示词", lines=3
        )
        with gr.Row():
        ...
        with gr.Row():
            sti_sm = gr.Checkbox(
                value=(
                    env.sm if env.model != "nai-diffusion-4-curated-preview" else False
                ),
                label="sm",
                visible=(
                    True if env.model != "nai-diffusion-4-curated-preview" else False
                ),
            )
        ...
        open_plugin_directory.click(
            open_folder,
            inputs=gr.Textbox(
                Path("./plugins/t2i/sanp_plugin_stories_to_images"), visible=False
            ),
        )
